...
if (!isset($_POST["qname"]) || !isset($_POST["qtext"]) || strlen(trim($_POST["qname"])) == 0 || strlen(trim($_POST["qtext"])) == 0) {
  echo "<center><h1>Required fields are not specified.</h1><br><h2><a href=\"ask.php\" class=\"btn btn-primary\">Back to ask form.</a></h2></center>";
  die();
}
session_start();
...
$result = $stmt->execute();
if ($result === FALSE) {
  echo "<center><h1>An error happened with the SQL query.</h1><h2>" . $conn->error . "</h2><br><h2><a href=\"ask.php\" class=\"btn btn-primary\">Back to ask form.</a></h2></center>";
  die();
}
...
if ($result === FALSE) {
  ...
  echo "<center><h1>Asking successful!</h1><br><h2><a href=\"index.php\" class=\"btn btn-primary\">Back to homepage.</a></h2></center>";
}
?>
